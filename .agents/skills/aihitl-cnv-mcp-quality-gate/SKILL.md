...
Work from `tools/CnvRegulation.McpServer`. Never print a DSN, password, API key, or secret-bearing environment value. Reset, corpus replacement, embedding rebuild, paid OpenAI, and default changes each need explicit authorization.
...
3. Run these read-only checks. Capture the identity row privately, compute SHA-256 over `database|user|server_addr|server_port`, and report only the fingerprint. Inspect pgvector/schema, stored model/dimension distribution, corpus, and coverage.

```powershell
psql -X -v ON_ERROR_STOP=1 -Atc "SELECT concat_ws('|',current_database(),current_user,coalesce(inet_server_addr()::text,'local'),coalesce(inet_server_port()::text,'local'))"
