...
3. Select the narrowest test filter or project build that proves the change. Assign a unique directory under `$env:TEMP` to `$outputPath` for this run.
4. Rerun the same target with `"-p:OutputPath=$outputPath" -m:1 -nr:false`.
...
if (-not (Test-Path (Join-Path $repoRoot 'python-agents'))) { throw 'Run from the ai-orchestration-hitl repository root.' }
...
| Python source/fixture resolution fails after redirect | Run `$env:ORCHESTRATION_TEST_PYTHON_HOME = Join-Path $repoRoot 'python-agents'` |
